...
#字符串的单个元素取值
#字符串里面的值取值出来 还是字符串 类型
#字符串里面元素是怎么定他们的位置的  索引
#索引：正序 ---> 从0开始 反序<----从-1开始
#字符串取值 是根据索引来取值的
#取值方式： 字符串变量名[索引值]
# print(p[-1])
...
#字符串切片:字符串变量名[m:n:k]
#m索引的起始位置  n 索引结束位置+1 k:步长
#k的默认值是1
# p_1='hello'#字符串
#请把p_1这个字符串里面的ll两个字母输出到控制台
# print(p_1[2:4])#取左不取右
#k默认值是1  对应的元素索引是：2 3 4  所以取值是ll
# print(p_1[0:5:2])
#k的值是2  每次索引取值会加2 对应的索引是 0 2 4
# print(p_1[2:8:3])
#k的值是3  每次索引取值会加3 对应的索引是 2 5
# print(p_1[:])#k=1 m=0 n=到末尾+1
# print(p_1[0:])
